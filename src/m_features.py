...
class Meta_F_Runner:
    ...
    def train(self, xx):
        ...
        self.optim = torch.optim.Adam(
            list(self.model.parameters()), lr=self.lr, weight_decay=self.weightDecay
        )
...
class Meta_Features:
    def __init__(
        ...
        self.tcn = Encoder_Decoder_TCN(
            tcn_feaures, input_length, hidden_layers=(128, 64)
        )
        self.lstm_att = Seq2seqAttn(
            tlen=input_length, in_di=lstm_att_features, first_hs=256, second_hs=256
        )
    ...
    def feed_inputs(self, tcn_in, lstm_att_in, gradient):
        ...
        self.runner_tcn.latent = self.runner_tcn.latent.unsqueeze(1).view(
            tcn_in.size(0), self.input_length, -1
        )
        ...
        print("\n start mlp:")
        mlp_in = torch.concat(
            (self.runner_lstm_att.latent, self.runner_tcn.latent), dim=2
        ).detach()
...
if __name__ == "__main__":
    ...
    mfeatures = Meta_Features(
        device, X_lstm_att.size(2), X_tcn.size(2), X_tcn.size(1), 3, 0.06, 0.03, 2
    )
