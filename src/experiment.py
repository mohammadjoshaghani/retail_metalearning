...
class Optimizer:
    ...
    def objective(self):
        wandb.init()
        epochs, weightDecay, lr = (
            wandb.config.epochs,
            wandb.config.weightDecay,
            round(wandb.config.learning_rate, ndigits=4),
        )
        modes_loss_rmse = {}
        ...
        for FH in [7, 4, 1]:
            ...
            valid_loss_rmse, test_loss_rmse = (
                modes_loss_rmse["valid"],
                modes_loss_rmse["test"],
            )
            wandb.log(
                {
                    f"valid_loss_rmse_FH{FH}": valid_loss_rmse,
                    f"test_loss_rmse_FH{FH}": test_loss_rmse,
                }
            )
        # Log model performance metrics to W&B
        wandb.log(
            {"tcn_mse": tcn_mse, "lstm_att_mse": lstm_att_mse, "mlp_mse": mlp_mse}
        )
...
sweep_id = wandb.sweep(sweep_configs, project=f"Retail_M_{ExpId}")
